...
# Sort by date_published, fall back to date_collected
_sort_date = case(
    (Article.date_published.isnot(None), Article.date_published),
    else_=Article.date_collected,
)


def _sort_key(a):
    return a.date_published or a.date_collected


@router.get("", response_model=PaginatedArticles)
def list_articles(after: int | None = None, region: str | None = None, db: Session = Depends(get_db)):
    """Return up to 25 articles, optionally filtered by region, sorted by date descending."""
    base = select(Article).options(joinedload(Article.source)).order_by(_sort_date.desc(), Article.id.desc())
    if after is not None:
        # Cursor is still ID-based for stable pagination
    ...
    if region:
        ...
        items = sorted(rows[:PAGE_SIZE], key=_sort_key, reverse=True)
        has_more = len(rows) > PAGE_SIZE
        next_cursor = min(a.id for a in items) if has_more and items else None
        total = db.scalar(
    ...
    combined = sorted(hu_rows[:hu_limit] + intl_rows[:intl_limit], key=_sort_key, reverse=True)
    ...
    if len(combined) < PAGE_SIZE:
        ...
        combined = sorted(combined + fill_rows[:PAGE_SIZE - len(combined)], key=_sort_key, reverse=True)
